...
> 当前版本：`2.0.1(8)`，即 `app/build.gradle.kts` 里的 `versionName`，版本号后面括号中的数字表示这是 2.0.1 下的第几次修改。Android 安装器还会显示 `versionCode`，那是给系统用的整数，不是第二个版本号。
...
4. 截一张图存到应用的外部缓存目录，切成 16×32 网格取色，把"蓝点够多"的相邻格子合并成连通块（任务卡标题常是英文、`+10` 也读不到控件，所以不看控件文字）；
...
7. 点击后等待「每日任务等待时间」（默认 5 秒），用全局手势返回奖励页，再找下一个，直到界面上没有「+10 蓝色按钮」为止（安全上限 20 个）；
8. 找不到可点的按钮时提示「每日活动已完成」。
...
- 每日活动**不依赖控件文字**：任务卡标题常是英文、`+10` 在控件树里读不到（实测必应 Rewards 页面如此），所以改为全屏 16×32 网格取色 + 连通域找"横向蓝色胶囊"，再用 OCR 确认按钮上的文字。相关阈值都在 `CheckInEngine`：`BTN_CELL_BLUE_MIN`（一格 9×9 采样里至少几个蓝点才算蓝）、`isButtonLike` 的尺寸/宽高比/上下边界、`isRewardText` 的文字判定；`ScreenAnalyzer.BLUE_MIN` 是单个像素的蓝度门槛。
