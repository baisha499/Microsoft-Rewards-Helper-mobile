...
3. 截图取色，在所有含 `+10` 文案的节点里只保留**蓝底**的（按钮外观是蓝色圆角胶囊 + 白色 `+10`；矩形按比例外扩后取 9×9 采样点，采到蓝色背景即算候选，能采到白字的优先），然后**随机挑一个**点击（页面上一般是 3 个卡片）；
...
- 每日活动的「蓝色 +10 按钮」靠截图取色判断：节点矩形按短边 20%（2~20px）外扩后取 9×9 采样点，采到偏蓝的采样点（`ScreenAnalyzer.BLUE_MIN`，蓝度 ≥ 25）即算候选，采到接近纯白的点（`ScreenAnalyzer.WHITE_MIN`）的候选优先；按钮形状和字体因机型而异，所以只认"蓝底 + 白色 +10 文案"这些通用特征，不写死尺寸和精确颜色。截图取色失败会退化为在所有 `+10` 节点里随机挑一个。
